    ...
    try:
        ...
        if data.nunique() <= 1:
            return {
                ...
                'ADF İstatistiği': None,
                'p-değeri': None,
                'Kritik Değerler': None,
...
def create_latex_table(results, sheet_name):
    ...
    if sheet_name == 'perakende':
        # Her sayfada gösterilecek maksimum satır sayısı
        rows_per_page = 40
...
\\caption{{Fuller (Dickey-Fuller) Durağanlık Testi Sonuçları - {sheet_name} ({page + 1}/{total_pages})}}
\\label{{tab:fuller_test_{sheet_name}_{page + 1}}}
...
"""
            ...
            for result in current_results:
                sanitized_variable = sanitize_latex_variable(result['Değişken'])
                if result['ADF İstatistiği'] is None:
                    table += f"{sanitized_variable} & {{--}} & {{--}} & {{--}} & {{--}} & {result['Sonuç']} \\\\\n"
                else:
                    table += f"{sanitized_variable} & {result['ADF İstatistiği']:.4f} & {result['p-değeri']:.4f} & "
                    table += f"{result['Kritik Değerler']['1%']:.4f} & {result['Kritik Değerler']['5%']:.4f} & "
                    table += f"{result['Sonuç']} \\\\\n"
...
"""
        ...
        # Diğer sayfalar için normal tablo
...
"""
        ...
        for result in results:
            sanitized_variable = sanitize_latex_variable(result['Değişken'])
            if result['ADF İstatistiği'] is None:
                latex_table += f"{sanitized_variable} & {{--}} & {{--}} & {{--}} & {{--}} & {result['Sonuç']} \\\\\n"
            else:
                latex_table += f"{sanitized_variable} & {result['ADF İstatistiği']:.4f} & {result['p-değeri']:.4f} & "
...
def main():
    try:
        ...
        for sheet_name in xl.sheet_names:
            ...
            if 'Yıl' in numeric_columns:
                numeric_columns.remove('Yıl')
